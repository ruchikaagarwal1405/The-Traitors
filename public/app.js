...
function tableHTML(opts = {}) {
  ...
  ${list.map((p, i) => {
    ...
    return `<div class="${c}" style="transform:rotate(${ang}deg) translateY(calc(-1 * var(--R))) rotate(${-ang}deg)" ${pickable && pickable(p) ? `data-pick="${p.id}"` : ''} ${opts.kick && !p.isHost && !p.empty ? `data-kick="${p.id}"` : ''}><div class="av">${p.empty ? '?' : p.bot ? '🤖' : esc(initials(p.name))}${p.isHost && S.phase === 'LOBBY' ? '<i class="crown">👑</i>' : ''}${m.badge ? `<i class="badge">${m.badge}</i>` : ''}${m.cnt ? `<i class="cnt">${m.cnt}</i>` : ''}</div><div class="pn">${p.empty ? '' : esc(p.name)}</div></div>`;
  }).join('')}
...
function avRow(list, opts = {}) {
  ...
  return `<div class="avrow ${size}">${list.map(p => { const m = marks[p.id] || {};
    ...
    return `<div class="${c}" ${pickable && pickable(p) ? `data-pick="${p.id}"` : ''}><div class="av">${p.bot ? "🤖" : esc(initials(p.name))}${m.badge ? `<i class="badge">${m.badge}</i>` : ''}${m.cnt ? `<i class="cnt">${m.cnt}</i>` : ''}</div><div class="pn">${esc(p.name)}</div></div>`; }).join('')}</div>`;
}
...
function rLobby() {
  ...
  ${need ? `<button class="btn gold" data-sheet="invite">📜 Fellow players ko invite karein</button>${host && n >= 2 && !S.players.some(p => p.bot) ? `<button class="btn ghost sm" id="addbots">🤖 ${6 - n} computer players add karein (try karne ke liye)</button>` : ''}` : host ? `<button class="btn gold" id="start">Game shuru karein</button>` : `<div class="wait">Sab players palace mein aa chuke. Host 👑 game shuru karega...</div>`}
  <div class="card"><div class="rowb"><h3 style="margin:0">Players</h3><span class="muted small">${n}/10</span></div>
  <div class="guests">${S.players.map((p, i) => `<div class="guest ${p.connected ? '' : 'off'} ${p.id === S.me.id ? 'me' : ''}" ${host && !p.isHost ? `data-kick="${p.id}"` : ''}><div class="av">${p.bot ? '🤖' : esc(initials(p.name))}</div><div class="gname">${esc(p.name)}${p.isHost ? ' <span class="tiny">👑</span>' : ''}${p.bot ? ' <span class="tiny muted">(computer)</span>' : ''}${p.id === S.me.id ? ' <span class="tiny muted">(tum)</span>' : ''}</div><div class="gamt">₹${st.stake}</div></div>`).join('')}
...
function rMission() {
  ...
  if (dead) body = `<div class="wait">Baaki players mission khel rahe hain...</div>`;
    ...
    if (pd.step === 2) { const sel = local.multi || []; body = S.mySub2 !== undefined ? waiting('Vote ho gaya', pd.doneIds, al) : `<div class="narr">Kiski zubaan ladkhadayi? (ek se zyada chun sakte ho)</div><div class="chips">${aliveP().filter(p => p.id !== me.id && !p.bot).map(p => `<button class="chip ${sel.includes(p.id) ? 'on' : ''}" data-tw="${p.id}">${esc(p.name)}</button>`).join('')}</div><div class="row"><button class="btn dark sm" id="twnone">Koi nahi atka</button><button class="btn sm" id="twgo" ${sel.length ? '' : 'disabled'}>${sel.length ? sel.length + ' atke — Lock' : 'Chuno'}</button></div>`; }
  ...
  const pickable = !dead && !done && ((pd.type === 'kadi' && (pd.step === 1 || pd.kadi === me.id)) || (pd.type === 'chup' && pd.step === 2 && S.mySub2 === undefined)) ? (p => p.alive && p.id !== me.id && !(pd.type === 'chup' && p.bot)) : null;
...
function bind() {
  ...
  on('#addbots', 'click', () => socket.emit('addBots'));
